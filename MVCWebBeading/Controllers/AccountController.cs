...
using System.IO;
namespace MVCWebBeading.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        //
        // POST: /Home/ProcessImage
        [HttpPost]
        [AllowAnonymous]
        public RedirectToRouteResult ProcessImage(SendImageViewModel model, string returnUrl)
        {
            if (!ModelState.IsValid)
            {
                return RedirectToAction( "ProcessImage", "Account" );
            }
            byte[] source = null;
            using (var binaryReader = new BinaryReader(model.File.InputStream))
            {
                source = binaryReader.ReadBytes(model.File.ContentLength);
            }
            Image<Bgr, byte> tempImage = null;
            using (var ms = new MemoryStream(source))
            {
                System.Drawing.Bitmap bitmap = (System.Drawing.Bitmap)System.Drawing.Image.FromStream(ms);
                tempImage = new Image<Bgr, byte>(bitmap);
            }
            Mat image = tempImage.Mat;
            int bixelWidth;
            int bixelHeight;
            int.TryParse(model.BixelHeight, out bixelHeight);
            int.TryParse(model.BixelWidth, out bixelWidth);
            string SelectedWay = Request.Form["SelectListWithWays"];
            ...
            TempData["image"] = layer.image;
            return RedirectToAction( "ProcessedImage", "Account" );
        }
        [AllowAnonymous]
        public ActionResult ProcessedImage()
        {
            ViewBag.Message = "Result image";
            ProcessedImageViewModel model = new ProcessedImageViewModel();
            Mat image = (Mat)TempData["image"];
            byte[] array = null;
            using (var ms = new MemoryStream())
            {
                image.ToImage<Bgr, Byte>().Bitmap.Save(ms, ImageFormat.Jpeg);
                array = ms.ToArray();
            }
            model.base64 = Convert.ToBase64String(array);
            return View(model);
        }
